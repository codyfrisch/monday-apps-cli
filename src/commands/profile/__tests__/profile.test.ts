...
type StoredConfig = { accessToken?: string; defaultProfile?: string; profiles: Record<string, string> };

const readTempConfig = (dir: string): StoredConfig => {
  return JSON.parse(readFileSync(join(dir, '.mappsrc'), 'utf8')) as StoredConfig;
};

const runCommand = async (
  CommandClass: new (argv: string[], config: Config) => Command,
  argv: string[],
  configDir: string,
) => {
