...
describe('AuthenticatedCommand first-run auth flow', () => {
  it('resolves profile and sets token after profile:add setup', async () => {
    ...
    getConfigDataByKeySpy.mockReturnValue(undefined as unknown as string);
    ...
    const runCommandMock = jest.spyOn(config, 'runCommand');
    ...
    expect(runCommandMock).toHaveBeenCalledWith('profile:add');
  ...
  it('sets token after init --local plaintext setup', async () => {
    ...
    expect(runCommandMock).toHaveBeenCalledWith('init', ['--local']);
